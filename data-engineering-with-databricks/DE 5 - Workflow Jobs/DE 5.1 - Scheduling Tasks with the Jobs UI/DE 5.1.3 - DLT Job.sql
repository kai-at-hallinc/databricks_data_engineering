...
CREATE OR REFRESH STREAMING LIVE TABLE sales_orders_raw
  COMMENT "The raw sales orders, ingested from retail-org/sales_orders." AS
SELECT *
FROM
  cloud_files(
    "${source}/retail-org/sales_orders/",
    "json",
...
CREATE OR REFRESH STREAMING LIVE TABLE customers
  COMMENT "The customers buying finished products, ingested from retail-org/customers." AS
SELECT *
FROM
  cloud_files(
    "${source}/retail-org/customers/",
    "csv"
  );
...
CREATE OR REFRESH STREAMING LIVE TABLE sales_orders_cleaned
  ...
  COMMENT "The cleaned sales orders with valid order_number(s) and partitioned by order_datetime." AS
SELECT
  f.customer_id,
  f.customer_name,
  ...
  c.loyalty_segment
FROM
  STREAM (LIVE.sales_orders_raw) f
...
CREATE OR REFRESH LIVE TABLE sales_order_in_chicago
  COMMENT "Sales orders in Chicago." AS
SELECT
  city,
  order_date,
  customer_id,
